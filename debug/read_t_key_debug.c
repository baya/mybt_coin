...
int main(int argc, char *argv[])
{
    ...
    build_db_key(&tkey, DB_TXINDEX, src, strlen(src));

    kyk_print_hex("tkey ", (uint8_t *)tkey.body, tkey.len);
    ...
    value = leveldb_get(db, read_opts, (char *)tkey.body, tkey.len, &vlen, &errptr);
