...
int main()
{
    ...
    uint8_t lkey = DB_LAST_BLOCK;
    char *value = NULL;
    char *valptr = NULL;
    size_t vlen = 0;
    int nFile = 0;

    kyk_print_hex("lkey ", &lkey, sizeof(lkey));
    ...
    value = leveldb_get(db, read_opts, (char *)&lkey, sizeof(lkey), &vlen, &errptr);
    check(errptr == NULL, "get value error: %s", errptr);
    kyk_print_hex("raw value ", (unsigned char*)value, vlen);

    valptr = value;
    beej_unpack((unsigned char*)valptr, "<L", &nFile);

    printf("nFile: %d (the last block file number used)\n", nFile);
