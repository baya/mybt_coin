#include <stdio.h>
#include <string.h>
#include <stdlib.h>

#include "kyk_buff.h"
#include "kyk_utils.h"
#include "kyk_mkl_tree.h"
#include "mu_unit.h"

/* 数据来源于: https://webbtc.com/block/00000000d1145790a8694403d4063f323d499e655c83426834d4ce2f8dd4a2ee.json */
/* mrkl_tree: [ */
/* "b1fea52486ce0c62bb442b530a3f0132b826c74e473d1f2c220bfa78111c5082", */
/* "f4184fc596403b9d638783cf57adfe4c75c605f6356fbc91338530e9831e9e16", */
/* "7dac2c5666815c17a3b36427de37bb9d2e2c5ccec3f8633eb91a4205cb4c10ff" */
/* ] */


void build_tx_buf_from_hex(struct kyk_bon_buff *tx_buf, const char *hexstr)
{
    tx_buf -> base = kyk_alloc_hex(hexstr, &tx_buf -> len);    
}

char *test_make_mkl_tree2()
{

    char *tx1_hex = "";
    char *tx2_hex = "";
    struct kyk_bon_buff buf_list[2];
    struct kyk_mkltree_level *leaf_level;
    struct kyk_mkltree_level *root_level;

    build_tx_buf_from_hex(buf_list, tx1_hex);
    build_tx_buf_from_hex(buf_list+1, tx2_hex);
    leaf_level = create_mkl_leafs(buf_list, 2);
    root_level = create_mkl_tree(leaf_level);

    uint8_t target_rt[MKL_NODE_BODY_LEN];
    kyk_parse_hex(target_rt, "7dac2c5666815c17a3b36427de37bb9d2e2c5ccec3f8633eb91a4205cb4c10ff");
    int res = kyk_digest_eq(root_level -> nd -> bdy, target_rt, MKL_NODE_BODY_LEN);
    mu_assert(res, "failed to get the correct merkle 2 root");

    return NULL;
}


char *all_tests()
{
    mu_suite_start();
    
    mu_run_test(test_make_mkl_tree2);
    
    return NULL;
}

MU_RUN_TESTS(all_tests);







    











