...
char *test_make_gens_block()
{
    ...
    //kyk_parse_hex(target_blk_hash, "00000044481397b22e0fae24e230fe6d81b4ce94c6b7bfb124e29861484115c5");
    mu_assert(kyk_digest_eq(blk_hd.blk_hash, target_blk_hash, sizeof(target_blk_hash)), "failed to get the correct block hash");

    uint8_t target_mkl_rt[32];
    kyk_parse_hex(target_mkl_rt, "b76d27da4abf50387dd70f5d6cc7e4df1d54722631cbbfdd292463df77aa0dbd");
    mu_assert(kyk_digest_eq(blk_hd.mrk_root_hash, target_mkl_rt, sizeof(target_mkl_rt)), "failed to get the correct merkle root");
